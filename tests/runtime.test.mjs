...
import { loadBrokerSession } from "../plugins/ai/scripts/lib/broker-lifecycle.mjs";
import { resolveStateDir } from "../plugins/ai/scripts/lib/state.mjs";

const ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");
const PLUGIN_ROOT = path.join(ROOT, "plugins", "ai");
const SCRIPT = path.join(PLUGIN_ROOT, "scripts", "ai-companion.mjs");
...
test("review rejects focus text because it is native-review only", () => {
  ...
  assert.match(result.stderr, /\/ai:adversarial-review focus on auth/i);
});
...
test("review accepts --background while still running as a tracked review job", () => {
  ...
  assert.match(status.stdout, /# AI Status/);
...
test("status shows phases, hints, and the latest finished job", () => {
  ...
  assert.match(result.stdout, /`\/ai:status review-live`<br>`\/ai:cancel review-live`/);
...
test("stop hook logs running tasks to stderr without blocking when the review gate is disabled", () => {
  ...
  assert.match(blocked.stderr, /\/ai:status/i);
  assert.match(blocked.stderr, /\/ai:cancel task-live/i);
});
...
test("stop hook does not block when Codex is unavailable even if the review gate is enabled", () => {
  ...
  assert.match(allowed.stderr, /Run \/ai:setup/i);
});
