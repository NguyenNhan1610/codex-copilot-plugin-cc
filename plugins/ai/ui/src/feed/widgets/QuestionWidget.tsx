...
export function QuestionWidget({ event }: { event: SessionEvent }) {
  ...
  return (
    <div style={{ border: "1px solid var(--accent-purple)", borderRadius: "var(--radius)", padding: "8px 12px", background: "var(--bg-panel)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        <HelpCircle size={14} style={{ color: "var(--accent-purple)" }} />
        <span style={{ fontWeight: 600, fontSize: "var(--text-sm)", color: "var(--accent-purple)" }}>Question</span>
      ...
      {options.length > 0 && (
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {options.map((opt, i) => (
            <span key={i} style={{ padding: "2px 8px", borderRadius: 10, background: "var(--bg-elevated)", border: "1px solid var(--border)", fontSize: "var(--text-sm)", color: "var(--text-secondary)" }}>
              {opt}
            </span>
          ))}
        </div>
